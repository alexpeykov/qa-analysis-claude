# EVP Bank Application Setup

This prompt is used to set up and rebuild the EVP Bank application running in the `evpbank` Docker container on the `ubuntu-remote` server.
...
```
In the evpbank container on ubuntu-remote, run these commands sequentially in the /home/app/src directory:
...
5. app/console maba:webpack:compile
...
- **Execution Method**: SSH to ubuntu-remote, then docker exec into evpbank container
